...
Generic EM acceleration algorithms from the `turboEM` package --- implemented in version $\geq$ 3.0 --- can speed this up:
```{r, echo = TRUE, message = FALSE}
...
```{r, echo = FALSE}
fit.list <- list(fit.glm, fit.cem, fit.cem.acc)
time.list <- list(t.glm, t.cem, t.cem.acc)
...
rownames(res) <- c("glm", "cem", "cem.acc")
res
```
